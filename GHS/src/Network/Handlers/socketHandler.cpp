...
void SOCKHAND::compileData(cmdData &data, char* buffer, size_t size) {
    ...
    switch(data.cmd) {
        ...
        case CMDS::GET_ALL: {
        Clock::DateTime* dtg = Clock::DateTime::getInstance();
        Clock::TIME* time = dtg->getTime();
        SOCKHAND::DHTmtx->lock();

        written = snprintf(buffer, size,  
        "{\"firmv\":\"%s\",\"id\":\"%s\",\"sysTime\":%zu,\"hhmmss\":%d:%d:%d,\
        \"timeCalib\":%d,\"re1\":%d,\"re2\":%d,\"re3\":%d,\"re4\":%d,\
        \"temp\":%.2f,\"hum\":%.2f,\"tempRelay\":%d,\"tempCond\"\
        ...
        Clock::DateTime* cal = Clock::DateTime::getInstance();
        cal->calibrate(data.suppData);

        written = snprintf(
            buffer, size, "Time Calibrated to %zu", 
            static_cast<size_t>(cal->getTime()->raw)
            );
        }
